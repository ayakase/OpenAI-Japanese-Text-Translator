...
def translate(input_path: str, output_path: str, from_lang: str, to_lang: str):
    ...
    with open(output_path, "w+") as output_file:
        ...
        total_lines = len(output_file.readlines())
    line_accumulator=""
    line_done = 0
    with open(input_path) as input_file:
        for line_num, line in enumerate(input_file, start=1):
        # if line != "\n" and is_need_translate(line, from_lang) and :
            line_accumulator+=(f"&{line_num}& {line}")
            if line != "\n" and is_need_translate(line, from_lang) and is_japanese(line) and is_valid_token_length(line_accumulator):
                try:
                    ...
                    continue
    print(line_done)
    if (int(line_done) < total_lines):
        print("Processing some remaining lines")
        input.seek(0)
        for line_num, line in enumerate(input, start=int(line_done)):
    ...
    print("Done!")
    print("100%")
    # print("lines" + str(len(line_accumulator.splitlines())))
    # print("length" + str(len(line_accumulator)))
    # print(line_accumulator)
    output.close()
    #call api
